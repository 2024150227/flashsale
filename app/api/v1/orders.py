...
async def create_order(
    ...
    try:
        ...
        result = order_service.create_order(user_id, order.product_id, order.session_id)
...
async def cancel_order(
    ...
    需要JWT认证，只能取消自己的订单
    条件：下单时间不超过24小时，订单状态为 pending 或 completed
    """
    ...
    try:
        ...
        if order.status not in ["pending", "completed"]:
            raise HTTPException(status_code=400, detail="当前订单状态不支持取消")
